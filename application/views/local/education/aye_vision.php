<div class="row my-3 p-10 bg-white border rounded">
    <div class="col-12 mb-4 d-flex justify-content-between align-items-center my-10">
        ...
        <div><span class='add_aye_and_vision_document_btn btn btn-light-primary btn-icon'><i class='fa fa-plus'></i></span></div>
    </div>
    <div class="col-12">
        <table class="table" id="aye_and_vision_document_tb">
            <thead>
                <th>Title(EN)</th>
                <th>Title(ES)</th>
                <th>URL(EN)</th>
...
<div class="modal fade" id="aye_and_vision_document_modal">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-light-primary" id="aye_and_vision_document_save">Save</button>
...
<div class="modal fade" id="aye_and_vision_upload_modal">
    <div class="modal-dialog">
        <div class="modal-content">
            <!-- Modal Header -->
            ...
            <div class="modal-body">
                ...
                <!-- Modal footer -->
                <div class="modal-footer">
                    <button type="button" class="btn btn-light-primary" id="aye_and_vision_upload_save">Save</button>
                    <button type="button" class="btn btn-light-danger" data-dismiss="modal">Close</button>
                </div>
            </div>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        // Document
        var aye_and_vision_document_tb = $('#aye_and_vision_document_tb').DataTable({
            "autoWidth": false,
            ...
            "columns": [{
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        return `
                        <div data-id = "` + row.id + `">
                            <span class="btn btn-icon btn-sm btn-light-primary edit_aye_and_vision_document_btn"><i class="fas fa-edit"></i></span>
                            <span class="btn btn-icon btn-sm btn-light-warning upload_aye_and_vision_document_btn"><i class="fas fa-upload"></i></span>
                            <span class="btn btn-icon btn-sm btn-light-danger  delete_aye_and_vision_document_btn"><i class="fas fa-trash"></i></span>
                        </div>
                        `
                    }
                }
        ...
        $(".add_aye_and_vision_document_btn").click(function() {
            ...
            $("#aye_and_vision_document_modal").modal("show")
        })

        $(document).on('click', '.edit_aye_and_vision_document_btn', function() {
        ...
        $(document).on('click', '#aye_and_vision_document_save', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data = "ok") {
                        ...
                        aye_and_vision_document_tb.ajax.reload()
                    } else {
                        toastr.success("Action Failed!")
                    }
                    $("#aye_and_vision_document_modal").modal("hide")
                },
                error: function(error) {
                    toastr.error("Error was occurred on the server!")
                }
        ...
        $(document).on('click', '.upload_aye_and_vision_document_btn', function() {
            $("#chosen_document_id").val($(this).closest('div').attr('data-id'))
            $("#aye_and_vision_upload_modal").modal("show")
        })

        $(document).on('click', '.delete_aye_and_vision_document_btn', function() {
        ...
        // upload file
        $("#aye_and_vision_upload_save").click(function() {
            var fd = new FormData()
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.msg == "ok") {
                        setTimeout(function() {
                            $("#aye_and_vision_document_tb").DataTable().ajax.reload()
                        }, 1000);
                        mynotify('success', 'Saved Successfully.')
                        $("#aye_and_vision_upload_modal").modal("hide")
                    } else {
