...
class Insurances extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->session->set_userdata("page_status", "insurance");

        $this->load->view('insurances', $data);
    }

    public function read()
    {
        $url = $this->config->item("medical_center_url") . "api/readInsuranceByClinic";
        $client = new Client([
            'timeout' => 1800,
            'connect_timeout' => 60,
        ]);
        ...
        $this->Insurance_model->refreshinsurance($res);
