            ...
            if ($forgot_type == 1) { // Forgot Account
                ...
                if ($patient[0]) { // exist patient
                    if ($this->session->userdata('ying') == md5($info['captcha'] . '.png')) { // correct captcha
                        ...
                        $this->email->subject('doctorpimentel.com - ' . $email_password[$patient[0]['language']]['t_pa_ep_subject']);
                        $this->email->message($html);
            ...
            if ($this->session->userdata('ying') == md5($captcha . '.png')) {
                ...
                if ($data['fname'] == '' || $data['lname'] == '' || $data['dob'] == '') {
                    ...
                    if ($result['message'] == 'success') {
                        ...
                        $acronym = $this->ContactInfo_model->readAcronym()['acronym'];
                        ...
                        $this->email->subject('Case #: ' . $case_number  . $acronym . ' doctorpimentel.com - PT Area Account Request');
                        $this->email->message($html);
                        $this->email->send();
